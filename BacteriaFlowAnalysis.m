...
% Final output: flow data and other metadata to .xlsx file of user's choice
    ...
    defaults = {'9-28-20 indent_channel_pre-dried_pAb-fp'};
    boxDims = [1 80];
    ...
    questTitle2 = 'Data Series';
    choice2 = menu(questTitle2,'DI','1%Saliva','0.1%SARS','1%SARS','5%SARS','10%SARS','Other');
    if choice2 == 1
        SampleType = 'DI';
    elseif choice2 == 2
        SampleType = '1%Saliva';
    ...
    dlgTitle = 'Data Series Number';
    prompt = {'Sample Number'};
    defaults = {'1'};
    boxDims = [1 80];
    answer = inputdlg(prompt,dlgTitle,boxDims,defaults);
    sam_num = answer{1};
    SampleType = strcat(SampleType,'_',sam_num);

    frameIncrement = 3;
    dlgTitle = 'Single Run Settings';
    prompt = {'Enter Video Name:','Threshold Level:','Beginning Time (s):','Duration (s):'};
    defaults = {file,'0.7','0','30'};
    boxDims = [1 80];
    answer = inputdlg(prompt,dlgTitle,boxDims,defaults);
    VIDEO_IN = answer{1};
